...
class ATBBankExporter extends ExcelExporter
{
    public function __construct()
    {
        ...
        $this->spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load(Storage::disk('local')->path('templates/payment_raport_atb.xls'));
    }
    ...
    public function save(): ATBBankExporter
    {
        ...
        $data_array = $this->recipients->map(function ($recipient, $i) {
            return [
                $i + 1,
                mb_strtoupper((string) $recipient->last_name) . ' ' . mb_strtoupper((string) $recipient->first_name) . ' ' . mb_strtoupper((string) $recipient->middle_name),
